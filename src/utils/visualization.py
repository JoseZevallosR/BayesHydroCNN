def plot_subplots(data, output_path='subplots_depth_vs_friction.png'):
    """
    Genera subplots para cada punto del shapefile, graficando:
    - Rugosidad (n) vs Profundidad Real (depth)
    - Rugosidad (n) vs Profundidad Predicha (depth_pred)
    Además, muestra:
    - Correlación (r) entre n y depth, n y depth_pred
    - RMSE entre depth y depth_pred
    """

    unique_points = list({(row, col, pid) for row, col, pid, *_ in data})
    num_points = len(unique_points)
    ncols = 4
    nrows = int(np.ceil(num_points / ncols))

    fig, axes = plt.subplots(nrows, ncols, figsize=(4 * ncols, 4 * nrows))
    axes = axes.flatten()

    for idx, (row, col, pid) in enumerate(unique_points):
        point_data = [(n, depth, depth_pred) for r, c, p, *_ , n, depth, depth_pred in data
                      if r == row and c == col and p == pid]
        
        if point_data:
            n_vals, depth_vals, depth_pred_vals = zip(*point_data)

            # Graficar puntos reales y predichos
            axes[idx].scatter(n_vals, depth_vals, alpha=0.7, edgecolors='k', label="Simulator", color='blue')
            axes[idx].scatter(n_vals, depth_pred_vals, alpha=0.7, edgecolors='k', label="Emulator", color='red')

            # Calcular métricas estadísticas
            try:
                r_real, _ = pearsonr(n_vals, depth_vals)
                r_pred, _ = pearsonr(n_vals, depth_pred_vals)
            except Exception:
                r_real, r_pred = np.nan, np.nan  # Si hay error en correlación (p. ej., por datos constantes)
            
            rmse = np.sqrt(mean_squared_error(depth_vals, depth_pred_vals))

            # Título y métricas
            axes[idx].set_title(f'ID {pid} RMSE(sim-emu)={rmse:.2f}\n'
                                f'r(n,sim)={r_real:.2f}, r(n,emu)={r_pred:.2f}')

            axes[idx].set_xlabel('Friction (n)')
            axes[idx].set_ylabel('Depth')
            axes[idx].legend()

    # Eliminar subplots vacíos
    for ax in axes[len(unique_points):]:
        fig.delaxes(ax)

    plt.tight_layout()
    fig.savefig(output_path, dpi=300)
    #print(f"Subplots saved to: {os.path.abspath(output_path)}")
    plt.show()

def plot_points_on_map_with_ids(tiff_path, shapefile_path, output_path='points_on_map.png'):
    """
    Plots observation points from a shapefile over a TIFF raster map, adds point IDs and scale bar.
    The output is formatted for scientific publication.

    Parameters:
        tiff_path (str): Path to the GeoTIFF raster file.
        shapefile_path (str): Path to the shapefile with observation points (must contain 'id' field).
        output_path (str): File path to save the output figure (PNG + TIFF).
    """
    gdf = gpd.read_file(shapefile_path)

    with rasterio.open(tiff_path) as src:
        fig, ax = plt.subplots(figsize=(10, 8))

        # Plot raster with 'terrain' colormap
        show(src, ax=ax, cmap='terrain')

        # Plot observation points
        gdf.plot(ax=ax, color='red', markersize=20, zorder=3)

        # Set font for publication (DejaVu Sans or Arial)
        plt.rcParams['font.family'] = 'DejaVu Sans'

        # Annotate each point with its ID
        for idx, row in gdf.iterrows():
            x, y = row.geometry.x, row.geometry.y
            point_id = row['id']
            ax.text(x, y, str(point_id), fontsize=12, ha='left', va='bottom', color='black', zorder=4)

        # Add title (Elsevier style)
        ax.set_title("(a) Observation points used for model validation", fontsize=14)

        # Add scale bar using resolution from raster
        try:
            resolution = src.res[0]  # Assuming square pixels
            scalebar = ScaleBar(dx=resolution, units='m', location='lower right', box_alpha=0.5)
            ax.add_artist(scalebar)
        except Exception as e:
            print("Warning: Scale bar could not be added:", e)

        # Remove axes ticks
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_xlabel("")
        ax.set_ylabel("")

        # Final layout and save
        plt.tight_layout()
        fig.savefig(output_path, dpi=600, bbox_inches='tight')  # PNG
        print(f"PNG map saved to: {os.path.abspath(output_path)}")

        # Save TIFF version
        tiff_output = output_path.replace(".png", ".tiff")
        fig.savefig(tiff_output, dpi=600, format='tiff', bbox_inches='tight')
        print(f"TIFF version saved to: {os.path.abspath(tiff_output)}")

        plt.show()

def calculate_pixel_correlation(X_train, y_train, output_path='correlation_heatmap.png'):
    """
    Computes the pixel-wise Pearson correlation between Manning’s roughness coefficient (n) and simulated water depth.
    Generates a high-quality heatmap suitable for scientific publication.

    Parameters:
        X_train (np.ndarray): Input array of shape (num_images, height, width, channels), with roughness in channel 0.
        y_train (np.ndarray): Target array of shape (num_images, height, width, channels), with depth in channel 0.
        output_path (str): File path where the heatmap image will be saved.
    """
    num_images, height, width, _ = X_train.shape
    correlation_matrix = np.full((height, width), np.nan)

    for row in range(height):
        for col in range(width):
            roughness_values = X_train[:, row, col, 0]
            depth_values = y_train[:, row, col, 0]
            if np.std(roughness_values) > 0 and np.std(depth_values) > 0:
                correlation_matrix[row, col] = np.corrcoef(roughness_values, depth_values)[0, 1]

    # Heatmap visual settings
    plt.figure(figsize=(10, 8))
    cmap = sns.diverging_palette(240, 10, as_cmap=True)  # perceptually uniform diverging colormap

    ax = sns.heatmap(
        correlation_matrix,
        cmap=cmap,
        center=0,
        vmin=-1,
        vmax=1,
        cbar_kws={'label': 'Pearson correlation (r)'},
        square=True,
        xticklabels=False,
        yticklabels=False
    )

    plt.title("(b) Spatial correlation between Manning’s n and water depth", fontsize=14)
    plt.xlabel("Column index (grid cell)", fontsize=12)
    plt.ylabel("Row index (grid cell)", fontsize=12)
    plt.tight_layout()

    plt.savefig(output_path, dpi=600, bbox_inches='tight')  # high-resolution for Elsevier
    print(f"Correlation heatmap saved at: {os.path.abspath(output_path)}")
    plt.show()

    return correlation_matrix

def get_n1_n2(X_val, indices):
    """
    Obtiene los dos valores más frecuentes (n1, n2) en el canal 0 de las imágenes en X_val.

    Parámetros:
    - X_val: numpy array de tamaño (num_samples, height, width, channels), imágenes de validación.
    - indices: lista o array con los índices de las imágenes a analizar.

    Retorna:
    - rugosities: array de tamaño (len(indices), 2) con (n1, n2) para cada imagen.
    """
    rugosities = []

    for idx in indices:
        # Extraer la imagen y su canal 0 (fricción)
        friction_values = X_val[idx, :, :, 0].flatten()

        # Contar las ocurrencias de cada valor único
        values, counts = np.unique(friction_values, return_counts=True)

        # Obtener los dos valores más frecuentes
        top_indices = np.argsort(-counts)[:2]  # Ordenar por frecuencia descendente
        n1, n2 = values[top_indices] if len(top_indices) == 2 else (values[0], values[0])  # Si solo hay un valor, repetirlo

        # Almacenar resultado
        rugosities.append((n1, n2))

    return np.array(rugosities)

def combine_images_side_by_side_same_size(img1_path, img2_path, output_path='combined_side_by_side.png'):
    """
    Combina dos imágenes horizontalmente (una al lado de la otra) asegurando que tengan la misma altura.
    La segunda imagen será redimensionada si es necesario.

    Parámetros:
        img1_path (str): Ruta de la primera imagen.
        img2_path (str): Ruta de la segunda imagen.
        output_path (str): Ruta donde se guardará la imagen combinada.
    """
    # Abrir las imágenes
    img1 = Image.open(img1_path)
    img2 = Image.open(img2_path)

    # Redimensionar img2 para que coincida con la altura de img1
    if img1.size[1] != img2.size[1]:
        new_height = img1.size[1]
        new_width = int((new_height / img2.size[1]) * img2.size[0])
        img2 = img2.resize((new_width, new_height), Image.Resampling.LANCZOS)

    # Crear una nueva imagen combinada
    total_width = img1.size[0] + img2.size[0]
    max_height = max(img1.size[1], img2.size[1])
    combined_img = Image.new('RGB', (total_width, max_height))

    # Pegar ambas imágenes
    combined_img.paste(img1, (0, 0))
    combined_img.paste(img2, (img1.size[0], 0))

    # Guardar resultado
    combined_img.save(output_path)
    print(f"Combined image saved to: {os.path.abspath(output_path)}")

def combine_subplots_only(test_img_path, train_img_path, output_path='combined_subplots_only.png'):
    """
    Combina las imágenes de subplots de test y train en una figura vertical.
    """
    # Cargar imágenes
    test_img = Image.open(test_img_path)
    train_img = Image.open(train_img_path)

    # Crear figura más grande y vertical
    fig, axes = plt.subplots(2, 1, figsize=(20, 16))  # ancho x alto

    # Subplot de test
    axes[0].imshow(test_img)
    axes[0].axis('off')
    axes[0].set_title('Test set', fontsize=18)

    # Subplot de train
    axes[1].imshow(train_img)
    axes[1].axis('off')
    axes[1].set_title('Training set', fontsize=18)

    plt.tight_layout()
    #fig.savefig(output_path, dpi=300)
    fig.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Combined figure saved to: {os.path.abspath(output_path)}")
    plt.show()

def save_error_maps(mae_pixel, rmse_pixel, medae_pixel, std_pixel, output_path="error_maps.png"):
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))
    #fig.suptitle("Distribución de Errores por Píxel", fontsize=16, fontweight="bold")

    metrics = [
        ("MAE (Mean Absolute Error)", mae_pixel, "inferno"),
        ("RMSE (Root Mean Squared Error)", rmse_pixel, "magma"),
        ("MedAE (Median Absolute Error)", medae_pixel, "plasma"),
        ("Global Std Dev of Error", std_pixel, "viridis")
    ]

    vmax_common = max(np.max(mae_pixel), np.max(rmse_pixel), np.max(medae_pixel))

    for ax, (title, data, cmap) in zip(axes.flat, metrics):
        im = ax.imshow(data, cmap=cmap, vmin=0, vmax=vmax_common if "STD" not in title else None)
        ax.set_title(title, fontsize=12)
        ax.axis("off")
        cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
        cbar.set_label("Valor del Error", fontsize=10)

    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.savefig(output_path, dpi=600)
    plt.close()